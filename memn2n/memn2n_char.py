...
class MemN2N_char(object):
    """End-To-End Memory Network."""

    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size, vocab_character_size,
                 character_size,
                 hops=3,
                 max_grad_norm=40.0,
                 nonlin=None,
    ...
    def batch_fit(self, stories, queries, answers, learning_rate, stories_char, queries_char):
        ...
        feed_dict = {self._stories_char: stories_char, self._queries_char: queries_char, self._stories: stories,
                     self._queries: queries, self._answers: answers, self._lr: learning_rate}
        # pdb.set_trace()
            ...
            for idx_sents, sents in enumerate(story):
                ...
                if len(position_list) > 0 and recognise:
                    for position in position_list:
                        similar_smaple_in_train_positions = similar_sample(tags_test[idx_story][idx_sents], tags_train,
                                                                           position)
                        # pdb.set_trace()
